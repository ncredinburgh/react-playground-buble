...
export const forceViewerUpdate = () => _forceUpdateCallbacks.forEach(fn => fn())
...
export default class PlaygroundViewer extends React.Component {
  ...
  renderDom = scope => transpiled =>
    new Promise((resolve, reject) => {
      ...
      if (React.isValidElement(answer)) {
        ...
        ReactDOM.render(wrapOutput(answer), this.el)
      }
      resolve(true)
    })
  ...
  componentWillUnmount() {
    _forceUpdateCallbacks.filter(x => x !== this.forceUpdate)
    ReactDOM.unmountComponentAtNode(this.el)
  }
